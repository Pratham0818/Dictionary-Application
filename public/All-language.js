...
document.getElementById('viewHistoryButton1').addEventListener('click', () => {
    ...
    function data(result, word) {
        if (result.title) {
            ...
            // Fetch translation for the meaning
            fetchTranslation(definitions.definition, word);
        }
    }
    ...
    // Function to fetch the translation of the word and its meaning
    function fetchTranslation(definition, word) {
        let wordTranslationUrl = `https://api.mymemory.translated.net/get?q=${word}&langpair=en|mr`; // Example: English to Marathi
        let meaningTranslationUrl = `https://api.mymemory.translated.net/get?q=${definition}&langpair=en|mr`; // Translate the definition

        // Fetch translation for the word
        fetch(wordTranslationUrl)
            .then(response => response.json())
            .then(data => {
                translation.innerText = `Marathi Translation of Word: ${data.responseData.translatedText}`;
            })
            .catch(() => {
        ...
        // Fetch translation for the meaning
        fetch(meaningTranslationUrl)
            .then(response => response.json())
            .then(data => {
                let translationContainer = document.querySelector(".translation .meaning-translation");
                if (!translationContainer) {
                    translationContainer = document.createElement("span");
                    translationContainer.classList.add("meaning-translation");
                    translation.insertAdjacentElement("beforeend", translationContainer);
                }
                translationContainer.innerText = `  Marathi Translation of Meaning: ${data.responseData.translatedText}`;
            })
            .catch(() => {
                let translationContainer = document.querySelector(".translation .meaning-translation");
                if (translationContainer) {
                    translationContainer.innerText = " | Translation not available.";
                }
            });
    }
...
document.getElementById('viewFavoritesButton').addEventListener('click', () => {
    window.location.href = 'All-Dic-favorites.html'; 
});


document.getElementById('wordOfTheDayButton').addEventListener('click', () => {
    window.location.href = 'word_of_the_day1.html'; 
});
